...
if options.solve == False:
    ...
    if notrace == False:
        print(f'{Colors.BOLD}=== Solving ==={Colors.END}\n')
        print(f'{Colors.BOLD}{"Access":<10} {"Result":<8} {"Status":<16} {"Re":<7} {"Hits":<7} {"Misses":<7}{Colors.END}')
        print('-' * 60)
    ...
    for nStr in addrList:
        ...
        if notrace == False:
            hit_color = Colors.GREEN if idx != -1 else Colors.RED
            mem_str = ', '.join(map(str, memory))
        
            # 对齐格式化输出
            print(f'{Colors.BOLD}-{Colors.END} {n:<8} ' 
                f'{hit_color}{hfunc(idx):<8}{Colors.END} '
                f'{mem_str:<16} '
                f'{Colors.YELLOW}{vfunc(victim):<8}{Colors.END}'
                f'{hits:<8}'
                f'{miss:<8}')
    ...
    # print('')
    hit_rate = (100.0 * float(hits)) / (float(hits) + float(miss))
    color = Colors.GREEN if hit_rate > 70 else Colors.RED if hit_rate < 30 else Colors.YELLOW
    
    print(f'\n{Colors.BOLD}=== FINALSTATS ==={Colors.END}')
    print(f'{Colors.BOLD}Hits:{Colors.END} {Colors.GREEN}{hits}{Colors.END}')
    print(f'{Colors.BOLD}Misses:{Colors.END} {Colors.RED}{miss}{Colors.END}')
    print(f'{Colors.BOLD}hitrate:{Colors.END} {color}{hit_rate:.2f}%{Colors.END}')
    print(f'{Colors.BOLD}polocy:{Colors.END} {Colors.BLUE}{policy}{Colors.END}')
    print(f'{Colors.BOLD}cache:{Colors.END} {cachesize} pages')
    print('-' * 30)
